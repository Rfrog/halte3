...
function BlowFish_Decode(PT: int64): int64;
function BlowFish_Encode(PT: int64): int64;
function BlowFish_Func(PT: LongWord): LongWord;
procedure BlowFish_Init(Key: PByte; KeyLength: Integer);
...
begin
  ...
  xR     := xL;
  xL     := tmp;
  xR     := xR xor Key_Boxes[ 2 ];
  xL     := xL xor Key_Boxes[ 1 ];
  r      := xL;
  r      := (r shl $20) or xR;
  Result := r;
end;
...
begin
  ...
  Result := ((Key_Boxes[ a + 19 ] + Key_Boxes[ b + 256 + 19 ]));
  Result := Result xor Key_Boxes[ c + 256 * 2 + 19 ];
  Result := Result + Key_Boxes[ d + 256 * 3 + 19 ];
end;
...
begin
  ...
  for i   := 1 to 4 do
    ...
    begin
      PT                                          := BlowFish_Encode(PT);
      Key_Boxes[ (i - 1) * 256 + j * 2 - 1 + 18 ] := PT shr $20;
      Key_Boxes[ (i - 1) * 256 + j * 2 + 18 ]     := PT mod $100000000;
    end;
