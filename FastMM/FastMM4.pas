...
Acknowledgements (for version 4):
 ...
 - G・ther Schoch for the "RequireDebuggerPresenceForLeakReporting" option.
 - Jan Schl・er for the "ForceMMX" option.
 ...
 - Marcus Mnig for the ScanMemoryPoolForCorruptions suggestion and the
...
Change log:
 ...
 - Added the "RequireDebuggerPresenceForLeakReporting" option to only display
   the leak report if the application is run inside the IDE. (Thanks to G・ther
   Schoch.)
 - Added the "ForceMMX" option, which when disabled will check the CPU for
   MMX compatibility before using MMX. (Thanks to Jan Schl・er.)
 ...
 Version 4.70 (4 August 2006):
  ...
  - Added a global variable "FullDebugModeScanMemoryPoolBeforeEveryOperation".
    ...
    when hunting for elusive memory corruption bugs. (Thanks to Marcus Mnig.)
